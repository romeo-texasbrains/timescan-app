...
interface ManagerProfileEditFormProps {
  profile: Profile & { departments?: { name: string } };
}

export default function ManagerProfileEditForm({ profile }: ManagerProfileEditFormProps) {
  ...
  const handleSubmit = async (formData: FormData) => {
    ...
    try {
      ...
      const response = await fetch(`/api/manager/profiles/${profile.id}`, {
        method: 'PUT',
        body: data,
      });
