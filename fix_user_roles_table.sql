-- This script checks if the user_roles table exists and creates it if it doesn't
-- It also ensures that all profiles have corresponding entries in the user_roles table
...
-- Now, populate the user_roles table with data from profiles
...
-- Check for any profiles that don't have a corresponding user_roles entry
SELECT p.id, p.full_name, p.email, p.role, p.department_id
FROM profiles p
LEFT JOIN user_roles ur ON p.id = ur.user_id
WHERE ur.user_id IS NULL;

-- Check for any user_roles entries that don't have a corresponding profile
SELECT ur.user_id, ur.role, ur.department_id
FROM user_roles ur
...
-- Check managers and their departments
